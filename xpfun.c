...
#define ELEMENT_HASH_SIZE 65536          // potencia de 2, por lo menos 256, tiene que caber en un UNSIGNED INT, (maximo es 65536; esta bien)
#define LOGICAL_TABLE_SOURCE_SIZE 131072 // tamaño de la lista de todos los table sources
...
int parse_header(struct Buffer *buf, FILE *oxml)
{
    ...
    char *header_end = strstr(buf->buffer, "<DECLARE>");
    if (header_end == NULL)
    {
    ...
    size_t header_size = header_end - buf->buffer;

    fwrite(buf->buffer, 1, header_size, oxml);
    fputc('\n', oxml);

    buf->cur = strchr(header_end, '<');
    return 1;
}
...
void logical_table_source_loop()
{
    ...
    for (size_t i = 0; i < logical_table_source_counter; i++)
    {
